...
use crate::{app::{App, FrameRange, View}, utils::load_video};
use eframe::egui::{self, ProgressBar};
use opencv::{imgcodecs, videoio::{self, VideoCaptureTrait}};
...
fn list_ui(ui: &mut egui::Ui, app: &mut App) {
    ui.horizontal(|ui| {
        if ui.button("Back").clicked() {
            app.current_view = View::Label;
        }
        if ui.button("Export").clicked() {
            app.export_progress = Some(export_labels(&app.video_path.clone().unwrap(), app.annotations.clone()));
            app.show_export_popup = true;
        }
    });
}

fn label_list(ui: &mut egui::Ui, app: &mut App) {
    let mut to_delete = None;
    for (i, annotation) in app.annotations.iter().enumerate() {
        ui.horizontal(|ui| {
            ui.label(&format!("{}: {} - {}", annotation.label, annotation.start_frame, annotation.end_frame));
...
fn export_labels_popup(_ui: &mut egui::Ui, app: &mut App, ctx: &egui::Context) {
    egui::Window::new("Export Labels")
        ...
        .show(ctx, |ui| {
            ...
            ui.add(ProgressBar::new(*progress_value).text(if *progress_value == 1.0 { "Done" } else { "Exporting labels..." }));
        }
...
fn export_labels(video_path: &PathBuf, annotations: Vec<FrameRange>) -> std::sync::Arc<std::sync::Mutex<f32>> {
    ...
    let export_dir = video_path
        .parent()
        .unwrap()
        .join(format!("{}_exported_frames", video_path.file_stem().unwrap().to_string_lossy()));
    ...
    std::thread::spawn(move || {
        ...
        let end_frame = annotations.iter()
